...
| File | Contents |
| --- | --- |
| `build/store-release-readiness-audit.json` | Strict local release audit evidence generated by the app-web release wrapper; currently 76 pass, 2 warn, 0 fail. |
...
| `build/store-release-artifacts.json` | Current clean app-web release artifact manifest for the branch head; includes strict audit summary, JavaScript backend, `flutter-web-build`, and `flutter-web-release-zip`. |
| `build/store-release-artifact-verification.json` | Strict app-web artifact verification evidence; currently 17 pass, 0 warn, 0 fail for the JavaScript web handoff artifacts. |
...
- GitHub release variables are present. External inbox proof remains separate
  from the authenticated in-app support request path.
- `public.support_requests` is applied on the linked Supabase project and live
  ...
  RLS.
- GitHub PR #15 is green and mergeable; Flutter CI and Web Pages readiness both
  passed.
...
1. Merge PR #15 and let the GitHub Pages workflow deploy the app-web build from `main`.
