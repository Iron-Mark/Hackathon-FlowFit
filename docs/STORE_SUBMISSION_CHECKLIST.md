...
- [ ] Android/iOS store artifact wrapper builds pass `-SupportEmailVerified` or
      set `FLOWFIT_SUPPORT_EMAIL_VERIFIED=true` only after an external test
      email proves that inbox can receive mail. The app-web MVP deploy can use
      `-AllowUnverifiedWebSupportEmail` because Help & Support submits
      authenticated in-app support requests.
...
The strict audit should pass before app-web MVP launch. Remaining support inbox
proof is a store-contact artifact requirement unless the in-app support queue is
removed or fails verification.
